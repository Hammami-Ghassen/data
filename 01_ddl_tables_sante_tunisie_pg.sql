-- ============================================================================
-- DDL Script - GRH Ministère de la Santé de Tunisie (PostgreSQL)
-- Generated: 2026-03-05 (PostgreSQL conversion)
-- Original: Oracle 11g, converted for PostgreSQL 14+
-- Adapted from GRH_BPC (Banque Postale du Congo) for 
-- the Tunisian Ministry of Health context
-- ============================================================================
-- Execution order: Run this script first, then 02_reference_data_sante_tunisie_pg.sql
-- ============================================================================
...
-- Changed: COD_GOUV kept VARCHAR(4) for consistency
...
CREATE TABLE GOUVERNORAT (
    ...
    LIB_GOUV_A    VARCHAR(40),                -- Libellé en arabe
...
CREATE TABLE DELEGATION (
    COD_GOUV      VARCHAR(4)  NOT NULL,
    COD_DELEG     VARCHAR(4)  NOT NULL,        -- Was COD_POST in POSTE table
    LIB_DELEG     VARCHAR(60) NOT NULL,        -- Was LIB_POST
    LIB_DELEG_A   VARCHAR(60),                 -- Was LIB_POST_A (libellé arabe)
    COD_DELEG_ADM VARCHAR(2),                  -- Was COD_DELEG (code admin délégation)
    COD_CIRC      VARCHAR(4),                  -- Was COD_CONT (code circonscription sanitaire)
...
CREATE TABLE LOCALITE (
    ...
    COD_DELEG     VARCHAR(4),                  -- Was COD_POST, now references DELEGATION
    LIB_LOC       VARCHAR(60) NOT NULL,
    LIB_LOC_A     VARCHAR(60),                 -- Libellé en arabe
...
-- ---------------------------------------------------------------------------
CREATE TABLE PAYS (
    COD_PAYS      VARCHAR(4)  NOT NULL,
    LIB_PAYS      VARCHAR(40),
    ABRV_PAYS     VARCHAR(4),
    CONTINENT     VARCHAR(1),
    LIB_PAYS_A    VARCHAR(40),
    ABRV_PAYS_A   VARCHAR(4),
    ABRV_PAYS1    VARCHAR(4),
    URL_DRAPEAU   VARCHAR(100),
...
CREATE TABLE TYP_CATEGORIE (
    TYP_CAT       VARCHAR(4)  NOT NULL,
    LIB_TYP_CAT   VARCHAR(60),
    LIB_TYP_CAT_A VARCHAR(60),                -- Libellé en arabe
...
CREATE TABLE TYPE_ORG (
    TYP_ORG        VARCHAR(4)  NOT NULL,
    LIB_TYP_ORG    VARCHAR(60),
    LIB_TYP_ORG_A  VARCHAR(60),
    RANG           SMALLINT,
...
CREATE TABLE ETAT_PAIE (
    COD_ETAT      VARCHAR(2)  NOT NULL,
    LIB_ETAT      VARCHAR(40),
    LIB_ETAT_A    VARCHAR(40),
...
-- ---------------------------------------------------------------------------
CREATE TABLE STATUT (
    COD_STAT      VARCHAR(4)  NOT NULL,
    LIB_STAT      VARCHAR(40) NOT NULL,
    TYPE_DECL     VARCHAR(10),
    LIB_STAT_A    VARCHAR(40),
...
CREATE TABLE ECHELON (
    COD_ECH       SMALLINT  NOT NULL,
    DUREE_ECH     SMALLINT,                  -- Durée en mois
    COD_CLASS     VARCHAR(4),
    LIB_ECH       VARCHAR(100),
...
CREATE TABLE LISTE_DOMAINE (
    ...
    LIB_DOMAINE    VARCHAR(50),
    LIB_DOMAINE_A  VARCHAR(50),
...
CREATE TABLE PRM_LIEU_GEOGRAPHIQUE (
    COD_LIEU_GEOG    VARCHAR(10) NOT NULL,
    LIB_LIEU         VARCHAR(100),
    LIB_LIEU_A       VARCHAR(100),
    DIST_LIEU_TUNIS  INTEGER,               -- Distance par rapport à Tunis (km)
    NBR_ASSOCIATION  INTEGER,               -- Nb d'associations dans chaque lieu
...
CREATE TABLE PRM_MILITAIRE (
    COD_MIL       VARCHAR(2)  NOT NULL,
    LIB_MIL       VARCHAR(80),
    LIB_MIL_A     VARCHAR(80),
...
CREATE TABLE GROUPE_SANGUIN (
    GRP_SANG        VARCHAR(4) NOT NULL,
    LIB_GRP_SANG    VARCHAR(30),
    LIB_GRP_SANG_A  VARCHAR(30),
...
CREATE TABLE PARAM_MVT (
    COD_MVT       VARCHAR(4)  NOT NULL,
    LIB_MVT       VARCHAR(60),
    LIB_MVT_A     VARCHAR(60),
    TYP_MVT       VARCHAR(2),                  -- Groupe: 1=recrutement, 2=position, 3=promotion, etc.
    COD_MVT_ANN   VARCHAR(4),                  -- Code mouvement d'annulation
...
CREATE TABLE PARAM_NIVEAU (
    COD_NIVEAU     VARCHAR(5)  NOT NULL,
    LIB_NIVEAU     VARCHAR(60),
    TYP_NIVEAU     VARCHAR(1),                 -- N=niveau, D=diplôme
    ETUDE          VARCHAR(3),                 -- PRO=professionnel, SEC=secondaire, UNI=universitaire
    CYCLE          VARCHAR(3),
    NAT_NIVEAU     VARCHAR(4),
    CODE_DOMAINE   VARCHAR(10),
    NBR_AN         NUMERIC(4,2),                 -- Nombre d'années d'études
    LIB_NIVEAU_A   VARCHAR(60),
    RANG           INTEGER,                 -- Rang du diplôme
...
CREATE TABLE FILLIERE (
    COD_FIL       VARCHAR(4)  NOT NULL,
    LIB_FIL       VARCHAR(100),
    CAT_DEB_FIL   VARCHAR(4),
    CAT_FIN_FIL   VARCHAR(4),
    LIB_FIL_A     VARCHAR(100),
    REG_TRAV      VARCHAR(4),
    TAUX_MAJ      NUMERIC(5,2),
...
CREATE TABLE NATURE_RECRUT (
    COD_NAT_RECR   VARCHAR(4) NOT NULL,
    LIB_NAT_RECR   VARCHAR(40),
    LIB_NAT_RECR_A VARCHAR(40),
    TYP_CAND       VARCHAR(1),                 -- E=externe, D=direct, R=réintégration, A=apprenti, P=parrainage
    TYP_CONT       VARCHAR(1),
    NAT_AFFECT     VARCHAR(1),
    COD_STAT       VARCHAR(4),
    NIV_SAL        NUMERIC,
    COD_ECH        NUMERIC,
...
CREATE TABLE MOTIF_SORT (
    COD_MOTIF       VARCHAR(4)  NOT NULL,
    LIB_MOTIF       VARCHAR(50),
    SALAIRE         VARCHAR(1),
    AVANCEMENT      VARCHAR(1),
    CONGE           VARCHAR(1),
    TYPE            VARCHAR(1),                -- E=entrée, S=sortie
    ETAT_ACT        VARCHAR(1),
    INCIDENCE       INTEGER,
    ASSURANCE       VARCHAR(1),
    INCIDE_ELECT    VARCHAR(1),
    NATURE_MOTIF    VARCHAR(1),                -- 1=Activité, 2=Détachement, 3=Sans solde, 4=SN, 5=Détach.interne, 6=Disponibilité, 7=Mise à disposition
    AVC_AID         VARCHAR(1),
    LIB_MOTIF_A     VARCHAR(50),
    TYP_IMPOT       VARCHAR(1) DEFAULT 'N',
    RESERVE         VARCHAR(1) DEFAULT 'O',
    NIV             INTEGER,
    COD_MVT         VARCHAR(4),
    COD_M           VARCHAR(4),
    SEXE            VARCHAR(1),
    DEL_DEM_RET     NUMERIC,
    AV_AP_DT_FIN    VARCHAR(1),
    DEM_REPRISE     VARCHAR(1),
    COD_MVT_REP     VARCHAR(4),
    COD_TYP_DEPART  VARCHAR(4),
    ACCORD_AUTORIT  VARCHAR(1),
    COD_ALERT       VARCHAR(4),
    DUREE_INSTANCE  INTEGER,
    NIV_PROLONG     INTEGER,
...
CREATE TABLE TYPE_DEPART (
    COD_TYP_DEPART    VARCHAR(4)  NOT NULL,
    LIB_TYP_DEPART    VARCHAR(100),
    LIB_TYP_DEPART_A  VARCHAR(100),
    DEM_TYP_DEPART    VARCHAR(4),
    COD_MVT            VARCHAR(4),
    NATURE_DEPART      VARCHAR(1),             -- D=démission, R=retraite, F=fin contrat, I=invalidité, S=disciplinaire, M=décès, A=abandon, E=enquête, P=période essai, C=licenciement, L=fin relation, N=négocié
    TAUX_PBI           NUMERIC(6,2),
...
CREATE TABLE AFFECTATION (
    COD_AFFECT     VARCHAR(4)  NOT NULL,
    LIB_AFFECT     VARCHAR(40) NOT NULL,
    REGIME         VARCHAR(1),                  -- M=mensuel, H=horaire
    JOURS          NUMERIC(4,2),
    HEURES         NUMERIC(7,3),
    RESERVE        VARCHAR(1),
    TYP_AFFECT     VARCHAR(1),
    LIB_AFFECT_A   VARCHAR(40),
    NAT_AFFECT     VARCHAR(1),                 -- T=titulaire, C=contractuel, S=stagiaire
    COD_MVT        VARCHAR(4),
...
CREATE TABLE FONCTIONS (
    COD_FONCT      VARCHAR(4)  NOT NULL,
    LIB_FONCT      VARCHAR(40) NOT NULL,
    TYP_FONCT      VARCHAR(1) DEFAULT 'F' NOT NULL, -- F=fonction, Q=qualification
    ABRV_FONCT     VARCHAR(20),
    NIVEAU         INTEGER,
    LIB_FONCT_A    VARCHAR(40),
    RESERVE        VARCHAR(1),                  -- P=nommé par décret, A=autres
    RESP_FONCT     VARCHAR(1),                  -- Fonction de responsabilité
    NB_SAL         NUMERIC,
    IND_INSAL      SMALLINT,
    STRATEGIQU     VARCHAR(1),                  -- O=stratégique, N=non
    ID_FONCTIONS   NUMERIC,
    NOTE           VARCHAR(1) DEFAULT 'O',      -- Notation (O/N)
...
CREATE TABLE AFFELIATION (
    COD_AFF       VARCHAR(4)  NOT NULL,
    LIB_AFF       VARCHAR(100) NOT NULL,
    ABRV_AFF      VARCHAR(10),
    TYP_AFF       VARCHAR(1),
    LIB_AFF_A     VARCHAR(100),
    ABRV_AFF_A    VARCHAR(10),
    RESERVE       VARCHAR(1),
    RIB           VARCHAR(20),
    NAT_AFF       VARCHAR(1),
...
CREATE TABLE CODE_AFFILIATION (
    TYP_AFFIL        VARCHAR(15),
    COD_AFFIL        VARCHAR(4),
    LIB_AFFIL        VARCHAR(80),
    LIB_AFFIL_A      VARCHAR(80),
    ID_CODE_AFFILIATION NUMERIC,
    AGE              NUMERIC,                    -- Nombre de mois (pour retraite)
...
CREATE TABLE ACTIVITE_FAMILLE (
    COD_ACTIVITE      VARCHAR(4)  NOT NULL,
    LIB_ACTIVITE      VARCHAR(40) NOT NULL,
    ALLOCATION        VARCHAR(1) NOT NULL,
    PARENTE_ACT       VARCHAR(1) NOT NULL,     -- E=enfant, C=conjoint
    PENSION           VARCHAR(1),
    LIB_ACTIVITE_A    VARCHAR(40),
    MNT_PENSION       NUMERIC(7,3),
    AGE               INTEGER,
    PRM_SCOL          NUMERIC(10,3),
    MIN_AGE           INTEGER,
    MAX_AGE           INTEGER,
    SCOL              VARCHAR(1),
    TYP_ACTIVITE      VARCHAR(4),
    ID_ACTIVITE_FAMILLE NUMERIC,
...
CREATE TABLE CATEG (
    ...
    LIB_CATEG       VARCHAR(60) NOT NULL,
    NBR_ECH         SMALLINT,
    TYP_CAT         VARCHAR(2),
    LIB_CATEG_A     VARCHAR(60),
    DUREE           BIGINT,               -- Durée de séjour dans la classification
    COD_GRAD_MIN    VARCHAR(4),
    COD_GRAD_MAX    VARCHAR(4),
    CATEG_COD_CATEG VARCHAR(4),
    COD_FIL         VARCHAR(4),                -- Code filière
    REGIME_TRAV     VARCHAR(1),
    REGIME_HEUR     VARCHAR(2),
    VPI             NUMERIC(7,3),                -- Valeur de point indiciaire
    SAL_MIN         NUMERIC(12,3),
...
CREATE TABLE CATEGORIE (
    ...
    LIB_CAT         VARCHAR(40),
    DUREE           SMALLINT,                -- Durée en mois
    NBR_ECH         SMALLINT,
    TYP_CAT         VARCHAR(2),
    NBR_PAIE        NUMERIC(5,2),
    ABS_CNG         VARCHAR(1),
    PRIME           NUMERIC(10,3),
    NOTE_REND       VARCHAR(1),                -- Notation O/N
    COD_GROUP       SMALLINT,
    QUALF           VARCHAR(10),
    PLAFOND_GRADE   NUMERIC,
    GR_COMP         VARCHAR(1) DEFAULT 'N',
    JOUR_DEMISSION  INTEGER,                -- Nb jours de préavis
    QUOTA_PBI       INTEGER,
    QUOTA_PBI_AG    INTEGER,
    QUOTA_ECH       INTEGER,
    VAR1_SOLUTION1  INTEGER,
    VAR1_SOLUTION2  INTEGER,
    VARIANTE2       INTEGER,
    PERIOD_ESSAI    INTEGER,                -- Nb mois période d'essai
    ID_CATEGORIE    NUMERIC,
    LIB_CAT_A       VARCHAR(40),
...
CREATE TABLE GRADE (
    ...
    LIB_GRAD        VARCHAR(60),
    ABRV_GRAD1      VARCHAR(10),
    ABRV_GRAD2      VARCHAR(20),
    DUREE           SMALLINT,                -- Durée de séjour
    NBR_ECH         SMALLINT,                -- Nombre des échelons
    COD_CAT_ELECT   VARCHAR(4),
    NOTE            VARCHAR(1),                -- Notation O/N
    COD_GROUP       SMALLINT,
    COD_CAT_RS      VARCHAR(4),
    CAT_RESULTAT_RS VARCHAR(4),
    LIB_GRAD_A      VARCHAR(60),               -- Libellé en arabe
    MNT_COMP        NUMERIC(15,3),
    COD_SOC         VARCHAR(4),
    TYP_CAT         VARCHAR(4),
    ID_GRADE        NUMERIC,
...
CREATE TABLE METIER (
    ...
    LIB_METIER     VARCHAR(60),
    LIB_METIER_A   VARCHAR(60),
    DESCRIP        VARCHAR(200),
...
CREATE TABLE POSTE_TRAV (
    ...
    POST_TYP       VARCHAR(1),                 -- F=fonctionnel, Q=non fonctionnel
    LIB_POST       VARCHAR(200),
    LIB_POST_A     VARCHAR(200),
    ABRV_POST      VARCHAR(10),
    ABRV_POST_A    VARCHAR(10),
    DESCRIP        VARCHAR(2000),
    DESCRIP_A      VARCHAR(2000),
    SECT_ACTIVI    VARCHAR(200),
    APP_NNP        VARCHAR(200),
    DOM_TRAV       VARCHAR(400),
    NATURE_POST    VARCHAR(1),                 -- Poste stratégique ou non
    SECT_ACTIVI_A  VARCHAR(200),
    APP_NNP_A      VARCHAR(200),
    DOM_TRAV_A     VARCHAR(400),
    REFERENTIEL    VARCHAR(4),
    R_COD_METIER   VARCHAR(4),
    R_COD_POST     VARCHAR(15),
    COD_CATEG      VARCHAR(4),
    COD_CAT        VARCHAR(4),
    COD_GRAD       VARCHAR(4),
    COD_FIL        VARCHAR(4),
...
CREATE TABLE SOCIETE (
    ...
    COD_SECT          VARCHAR(4),
    COD_POST          VARCHAR(5)   NOT NULL,    -- Code postal
    LIB_SOC           VARCHAR(100) NOT NULL,
    ABRV_SOC          VARCHAR(20),
    RUE_SOC           VARCHAR(40),
    VILLE_SOC         VARCHAR(20),
    PAYS_SOC          VARCHAR(20),
    TEL_SOC           VARCHAR(10),
    FAX_SOC           VARCHAR(10),
    COD_TVA           VARCHAR(20),
    NUM_RETR          VARCHAR(20),              -- N° affiliation CNRPS
    JOUR_TRAV         NUMERIC(4,2),
    HEUR_TRAV         NUMERIC(5,2),
    HEUR_HEBD         NUMERIC(5,2),
    DEB_CYCLE         VARCHAR(4),
    FIN_CYCLE         VARCHAR(4),
    REP_WEB           VARCHAR(1),
    LIB_SOC_A         VARCHAR(100),             -- Libellé en arabe
    NOM_SIG1          VARCHAR(200),
    NOM_SIG2          VARCHAR(200),
    REPOS             VARCHAR(1),
    NAT_ARROND        VARCHAR(1),
    VAL_ARROND        NUMERIC(4,1),
    TYP_SOC           VARCHAR(1),
    ABRV_SOC_A        VARCHAR(20),
    RUE_SOC_A         VARCHAR(40),
    VILLE_SOC_A       VARCHAR(20),
    PAYS_SOC_A        VARCHAR(20),
    WEB               VARCHAR(1),
    NUM_RETR_COMP     VARCHAR(20),
    ACCT_SOC          VARCHAR(20),
    SOC_COD_SOC       VARCHAR(4),
    JJ_DEB            SMALLINT,
    MAX_AGE           INTEGER,
    MIN_AGE           INTEGER,
    MINISTERE_SOC     VARCHAR(100),             -- Ministère de tutelle
    COD_RETR          VARCHAR(5),
    ADR_ELC           VARCHAR(150),
    ACT_SOC           VARCHAR(40),
    NAT_IMPOT         SMALLINT,
    TAUX_CHARG_PROF   NUMERIC(5,2),
    PLAF_CHARG_PROF   NUMERIC(10,3),
    IETUD             NUMERIC(10,3),
    IHAND             NUMERIC(10,3),
...
CREATE TABLE SERVICE (
    ...
    SER_COD_SERV    VARCHAR(10),               -- Code service niveau supérieur
    LIB_SERV        VARCHAR(120) NOT NULL,
    ABR_SERV        VARCHAR(10),
    TYPE_SERV       VARCHAR(4),                 -- Niveau de service (voir TYPE_ORG)
    LIB_SERV_A      VARCHAR(120),               -- Libellé en arabe
    ABR_SERV_A      VARCHAR(10),
    TYPE_SERV_CHEF  VARCHAR(1),
    DEPT_ANAL       VARCHAR(4),
    COURANT         VARCHAR(1),                 -- Utilisable O/N
    ORDRE           INTEGER,
    ACTIVITE_SERV   VARCHAR(1),                 -- Service extérieur O/N
    REGIME_SERV     SMALLINT,
    COD_SECT        VARCHAR(10),
    NB_SAL          NUMERIC,
    REG_POST        VARCHAR(1),
    COD_REG         VARCHAR(4),
    PDMM            VARCHAR(1),
    TAUX_DEF        NUMERIC(7,3),
    DROIT_ASTR      VARCHAR(1) DEFAULT 'N',     -- Droit à l'astreinte
    TEL_ASTR        VARCHAR(15),
    ORG_SERV        VARCHAR(15),
    PRIM_ADM_PARA   VARCHAR(1),
    PRM_MAIT_CHANT  VARCHAR(1) DEFAULT 'N',
    PRM_MAIT_MAINT  VARCHAR(1) DEFAULT 'N',
    COD_FONCT       VARCHAR(10),               -- Fonction de responsable de structure
    TYP_FONCT       VARCHAR(1),
    MAT_PERS        VARCHAR(10),               -- Matricule de responsable
    DAT_AFFECT      DATE,
    TYP_AFFECT      VARCHAR(1),                -- V=affectation, I=intérim
    COD_LIEU_GEOG   VARCHAR(4),
    COD_CENTRE      VARCHAR(4),                -- Centre payeur
    COD_ETAB        VARCHAR(10),               -- Établissement sanitaire
    CPT_FRAIS       VARCHAR(10),
    SERV_CPT        VARCHAR(10),
    SERV_PAYEUR     VARCHAR(10),
    ID_SERVICE      NUMERIC,
...
CREATE TABLE PERSONNEL (
    ...
    NOM_PERS        VARCHAR(40),
    PREN_PERS       VARCHAR(40),
    NOM_PERS_A      VARCHAR(60),               -- Nom en arabe
    PREN_PERS_A     VARCHAR(60),               -- Prénom en arabe
    NOM_JF          VARCHAR(80),               -- Nom de jeune fille
    NOM_JF_A        VARCHAR(40),
    CIN             VARCHAR(20),               -- N° CIN
    SEXE            VARCHAR(1),
    COD_SIT         VARCHAR(1),                -- Situation familiale: C/D/V/M/R
    DAT_SIT         DATE,
    CHEF_FAM        VARCHAR(1),                -- Chef de famille O/N
    NBRE_ENF        SMALLINT DEFAULT 0,
    CHARG_ENF       VARCHAR(4),                -- Nb enfants à charge
    CHARG_ALL       VARCHAR(4),                -- Nb enfants (alloc. familiales)
    FCT_FAM         VARCHAR(1),                -- Conjoint actif O/N
    DAT_ENT         DATE,                       -- Date d'entrée
    DAT_TIT         DATE,                       -- Date titularisation
    COD_SERV        VARCHAR(10),               -- Code service
    COD_FONCT       VARCHAR(4),                -- Code fonction
    COD_CATEG       VARCHAR(4),                -- Classification niveau 1
    COD_CAT         VARCHAR(4),                -- Classification niveau 2
    COD_GRAD        VARCHAR(4),                -- Classification niveau 3 (grade)
    COD_MOTIF       VARCHAR(4),                -- Position administrative
    COD_NATP        VARCHAR(4),                -- Code nationalité
    COD_STAT        VARCHAR(4) DEFAULT '01',   -- Statut d'imposition
    DAT_SERV        DATE,
    DAT_FONCT       DATE,
    DAT_QUALF       DATE,
    DAT_CATEG       DATE,
    ...
    ETAT_ACT        VARCHAR(1),                -- État paie: 0=actif, 1=bloqué, 5=arrêt, 8=STC
    PER_MAT_PERS    VARCHAR(10),               -- Ancien matricule
    QUALF           VARCHAR(4),
    COD_ECH         SMALLINT,                -- Code échelon
    COD_AFFECT      VARCHAR(4),                -- Statut administratif
    COD_NIVEAU      VARCHAR(5),                -- Niveau d'instruction/diplôme
    POSTE_TRAV      VARCHAR(15),               -- Poste de travail (réf. POSTE_TRAV)
    COD_METIER      VARCHAR(4),
    COD_USER        VARCHAR(10),
    DAT_MAJ         DATE,
    ADM_TECH        VARCHAR(10),               -- Emploi fonctionnel (réf. TYP_ADM_TECH)
    DAT_STAT        DATE,
    DAT_CIN         DATE,                       -- Date de délivrance CIN
    LIE_EMI_CIN     VARCHAR(100),              -- Lieu de délivrance CIN
    DAT_AFFECT      DATE,
    LIEU_NAIS       VARCHAR(200),              -- Lieu de naissance
    COD_LIEU_GEOG   VARCHAR(10),               -- Lieu géographique
    DAT_LIEU_GEOG   DATE,
    GRP_SANG        VARCHAR(3),                -- Groupe sanguin
    NUM_RETR        VARCHAR(12),               -- N° CNRPS/CNSS
    COD_FIL         VARCHAR(4),                -- Filière
    DAT_FIL         DATE,
    SERV_MIL        VARCHAR(2),                -- Service militaire
    CODE_DOMAINE    VARCHAR(10),               -- Spécialité
    COD_UR          VARCHAR(10),
    COD_CLASS       VARCHAR(4),
    DAT_CLASS       DATE,
    COD_TYP_DEPART  VARCHAR(4),                -- Type de départ
    DAT_DEPART      DATE,
    DAT_UR          DATE,
    ORG_SERV        VARCHAR(15),               -- Affectation fonctionnelle
    DAT_POSTE_TRAV  DATE,
    COD_NAT_RECR    VARCHAR(4),                -- Nature de recrutement
    DAT_ADM_TECH    DATE,
    DAT_ORG_SERV    DATE,
    MAINTIEN_PERS   VARCHAR(1),
    MAINTIEN_DATE   DATE,
    TYP_RANG        VARCHAR(1),
    HANDICAP        VARCHAR(1) DEFAULT '0',    -- 0=non, 1=normal, 2=prioritaire, 3=avec accompagnant
    POURCENT_HAND   NUMERIC(5,2) DEFAULT 0,      -- Taux IPP
    TYP_HANDICAP    VARCHAR(1),                -- 0=physique, 1=intellectuelle, 2=vision, 3=oreille
    NUM_FICH_HAND   VARCHAR(10),
    NIV_SAL         INTEGER,                -- Indice salarial
    COD_ASSUR       VARCHAR(4) DEFAULT '001',
    NUM_ASSUR       VARCHAR(20),
    COD_RETR        VARCHAR(5),
    TYP_ID          VARCHAR(1),                -- Type identifiant: CIN, Passeport
    ETAT_SANTE      VARCHAR(1),                -- S=sain, H=handicapé
    DAT_EFF_FICH_HAND DATE,
    DAT_FIN_FICH_HAND DATE,
    REF_FONCT       VARCHAR(20),               -- Référence JORT
    NAT_TEXTE_FONCT VARCHAR(2),
    NOUV_DAT_ECH    DATE,
    DAT_NIV_SAL     DATE,
    DAT_ASS         DATE,
    DAT_AFF_CNAM    DATE,
    ETAT_POSTE_TRAV VARCHAR(1),                -- O=confirmé, N=intérim, A=avantage
    DAT_FIN_CONT    DATE,
    PRESUM_NAIS     VARCHAR(1) DEFAULT 'N',    -- Date naissance présumée O/N
    COD_FILL        VARCHAR(5),
    SUSPENS_ASS     VARCHAR(1),
    COD_MOTIF_SUSP  VARCHAR(4),
    DAT_FIN_SUSPENS DATE,
    DAT_DEB_SUSPENS DATE,
    CHRONIQUE       VARCHAR(1),
    COD_CAT_CLASS   VARCHAR(4),
    COD_GOUV        VARCHAR(4),                -- Gouvernorat (adresse)
    COD_LOC         VARCHAR(4),                -- Localité (adresse)
    NUM_ASS_GAT     VARCHAR(10),
    NUM_ACC         VARCHAR(20),               -- N° accident de travail
    CATEG_EMB       VARCHAR(4),
    CAT_EMB         VARCHAR(4),
    GRAD_EMB        VARCHAR(4),
    ECH_EMB         SMALLINT,
    ID_PERSONNEL    NUMERIC,
    LIEU_NAIS_A     VARCHAR(200),              -- Lieu naissance en arabe
    BENEF_ALLF      VARCHAR(4),
    COD_DIR         VARCHAR(4),
    COD_UF          VARCHAR(10),
...
CREATE TABLE TYP_ADM_TECH (
    ADM_TECH         VARCHAR(10) NOT NULL,
    LIB_ADM_TECH     VARCHAR(40),
    ABRV_ADM_TECH    VARCHAR(20),
    LIB_ADM_TECH_A   VARCHAR(40),
    ABRV_ADM_TECH_A  VARCHAR(20),
    TYP_ADM_TEC      VARCHAR(1),
    COD_GRAD         VARCHAR(4),
    H_DEROG          NUMERIC(5,2),               -- Coefficient prime enveloppe
    COD_CAT          VARCHAR(4),
    COD_CATEG        VARCHAR(4),
    COD_FIL          VARCHAR(4),
    POST_ORGANIQUE   VARCHAR(1),               -- O=organique, N=non organique
    COD_POST         VARCHAR(15),              -- Réf. POSTE_TRAV (NOT DELEGATION!)
    NIVEAU           VARCHAR(1),
    DROIT_VEHICUL    VARCHAR(1),
...
CREATE TABLE FAMILLE (
    NUM_FAM         SMALLINT  NOT NULL,
    PARENTE_ACT     VARCHAR(1),                -- E=enfant, C=conjoint
    COD_ACTIVITE    VARCHAR(4),
    PRENOM          VARCHAR(35) NOT NULL,
    PARENTE         VARCHAR(1)  NOT NULL,       -- E=enfant, C=conjoint, M=mère, P=père, D=divers, V=veuf
    DAT_MAR         DATE,
    NOM_JF          VARCHAR(40),
    DAT_NAISS       DATE,
    SEXE            VARCHAR(1),
    BENEF           VARCHAR(1),                -- Bénéficiaire O/N
    REGIME_CONJ     VARCHAR(5),
    DAT_DECE        DATE,
    MAT_CONJ        VARCHAR(10),
    PLAF_TOT_REMB   NUMERIC(10,3),
    COD_SOC         VARCHAR(4)  NOT NULL,
    MAT_PERS        VARCHAR(10) NOT NULL,
    ...
    COD_SIT         VARCHAR(1),
    DAT_EFFET_ACT   DATE,
    NUM_ASS_CONJ    VARCHAR(10),
    DAT_ENGAG_PARENT DATE,
    DAT_E_SAIS      DATE,
    DATE_VALIDATION  DATE,
    DECLAR_ALL      VARCHAR(1),
    PROCED_JUDIC    VARCHAR(1),
    PENSION         VARCHAR(1),
    ETAT_FAM        VARCHAR(1),
    MERE            SMALLINT,
    GRP_SANG        VARCHAR(3),
    PRENOM_A        VARCHAR(35),
    NOM_JF_A        VARCHAR(35),
    ETAT_VALID      VARCHAR(1),                -- V=validé, S=instance
    PEC             VARCHAR(1),                -- O=pris en charge, N=non
    CIN             VARCHAR(20),
    MNT_PENSION     NUMERIC(10,3),
    DAT_DIVORCE     DATE,
    PEC_MUT         VARCHAR(1),
    NOM_PREN        VARCHAR(60),
    NUM_FICH_HAND   VARCHAR(10),
    DAT_EFF_FICH_HAND DATE,
    DAT_FIN_FICH_HAND DATE,
    TYP_MEMBRE      VARCHAR(1),                -- F=fils, P=parrain
    CIN_CONJ        VARCHAR(20),
    PRENOM_MERE     VARCHAR(35),
    NUM_MERE        SMALLINT,
    COD_AFF_CSS     VARCHAR(4),
    DAT_CIN         DATE,
    LIEU_CIN        VARCHAR(50),
    DAT_PEC         DATE,
    HANDICAP        VARCHAR(1),
    DAT_PEC_MUT     DATE,
    PRESUM_NAIS     VARCHAR(1),
    OBSERVATION     VARCHAR(100),
    CHRONIQUE       VARCHAR(1),
    TYP_CIN         VARCHAR(1),                -- 1=CIN, 2=Passeport, 3=Carte séjour
    ID_FAMILLE      NUMERIC,
    COD_USER        VARCHAR(10),
    COD_PAYS        VARCHAR(4),
...
CREATE TABLE ADR_PERS (
    COD_GOUV         VARCHAR(4),
    COD_DELEG        VARCHAR(4),               -- Code délégation (compatible FK DELEGATION)
    COD_POST         VARCHAR(4),               -- Code postal (NOT FK to DELEGATION)
    COD_SOC          VARCHAR(4)  NOT NULL,
    MAT_PERS         VARCHAR(10) NOT NULL,
    ...
    TEL_PERS         VARCHAR(30),
    FAX_PERS         VARCHAR(15),
    ADR_COURANT      VARCHAR(1),               -- Adresse courante O/N
    ADR_ELECTRONIQUE VARCHAR(100),
    TEL_PORT_PERS    VARCHAR(30),
    NUMERO_ADR       NUMERIC,
    ETAT_ADR         VARCHAR(1),
    IMMB_PERS        VARCHAR(4),
    APP_PERS         VARCHAR(4),
    LOCAL_PERS       VARCHAR(4),
    ID_ADR_PERS      NUMERIC,
    REC_MAIL         VARCHAR(1) DEFAULT 'O',
    COD_USER         VARCHAR(10),
...
CREATE TABLE RENSEIGNEMENT_PERS (
    ...
    REG_CNG          VARCHAR(4),
    COD_CYCL         VARCHAR(4),
    SAL_NEG          NUMERIC(12,3),               -- Salaire hors grille
    TAUX_IMP         NUMERIC(5,2),
    REGIME_TRAV      VARCHAR(1),                -- M=mensuel, H=horaire
    DROIT_LAIT       SMALLINT,
    COD_REG          VARCHAR(4),
    NAT_REG          VARCHAR(1),
    COMP_CNG         VARCHAR(2),
    CODE_COMP_CNG    VARCHAR(2),
    REG_POST         VARCHAR(1),
    COD_EQUIP        VARCHAR(1),
    POINTE           VARCHAR(1) DEFAULT 'O',    -- Pointage O/N
    ANC_PERS         INTEGER,                -- Ancienneté en mois
    REGIME_HEUR      INTEGER,
    PERS_CLOG        VARCHAR(1) DEFAULT '0',    -- Prime de logement
    PERS_CELEC       VARCHAR(1),
    PERS_CVOIT       VARCHAR(1),
    PERS_CEAU        VARCHAR(1),
    CODE_CCHAUF      VARCHAR(1),
    CODE_RESIDENCE   VARCHAR(4),
    PRIM_ADM_PARA    VARCHAR(1) DEFAULT '0',
    CODE_PELERINAGE  VARCHAR(1),
    COEF_PTRIM       VARCHAR(1),
    TAUX_IMP_PTRIM   NUMERIC(10,3),
    ART40            VARCHAR(1) DEFAULT 'N',
    COD_GRP          VARCHAR(2),
    COD_SGRP         VARCHAR(4),
    IND_DOMEST       VARCHAR(1) DEFAULT '0',
    DAT_ART40        DATE,
    UNIFORM          VARCHAR(1) DEFAULT '0',    -- 0=sans, 1=uniform, 2=prime uniform
    ANC_PERS_TIT     INTEGER,                -- Ancienneté titularisation
    COD_GROUP        VARCHAR(10),
    NUM_POINTEUSE    VARCHAR(10),
    POINTE_LIBRE     VARCHAR(1),
    COD_GRP_POINT    VARCHAR(10),
    SIGNATURE        VARCHAR(1),
    COD_BAREME       VARCHAR(4),
    DAT_DEB_ASSUR    DATE,
    DAT_FIN_ASSUR    DATE,
    F_MOJHD_CHEHID   VARCHAR(1),               -- Fils de moudjahid/chahid
    IEP_MOJHD_CHEHID INTEGER,
    IEP_HORS_BEA     INTEGER,
    CONTRAT_FIDELITE  VARCHAR(1),
    DT_DEB_FIDELITE   DATE,
    DT_FIN_FIDELITE   DATE,
    COD_MEMB         VARCHAR(4),
    MNT_PARAM        NUMERIC(12,3),
    BON_REST         VARCHAR(1) DEFAULT 'O',
    COMPTE_PROCED_JUDIC VARCHAR(20),
    BENEF_PROCED_JUDIC  VARCHAR(50),
    BENEF            VARCHAR(50),
    COMPTE_HERIT     VARCHAR(20),
    NOM_PERS_CNSS    VARCHAR(100),              -- Nom de l'agent à la CNRPS/CNSS
    AVC              NUMERIC,
    DAT_AVQ          DATE,
    AVQ              NUMERIC,
    DAT_AVC          DATE,
...
CREATE TABLE COMPTE_PERS (
    COD_SOC          VARCHAR(4)  NOT NULL,
    MAT_PERS         VARCHAR(10) NOT NULL,
    COD_BANQ         VARCHAR(4)  NOT NULL,
    COD_AGC          VARCHAR(5)  NOT NULL,
    COD_TYP_BUL      VARCHAR(4)  NOT NULL,
    CPT_BANQ_PERS    VARCHAR(25) NOT NULL,      -- RIB
    ETAT_DOM         VARCHAR(1) DEFAULT 'N',
    NUM_DOM          VARCHAR(20),
    COMPT_COU        VARCHAR(1),
    COD_BANQ_SOC     VARCHAR(4),
    COD_AGC_SOC      VARCHAR(5),
    ID_COMPTE_PERS   NUMERIC,
    DAT_DOM          DATE,
    COD_USER         VARCHAR(10),
...
CREATE TABLE CONTRAT (
    COD_SOC          VARCHAR(4)  NOT NULL,
    MAT_PERS         VARCHAR(10) NOT NULL,
    NUM_CONTRAT      NUMERIC       NOT NULL,
    COD_TYP          VARCHAR(4)  NOT NULL,
    ETAT             VARCHAR(1),
    NAT_CONTRAT      VARCHAR(4),
    NUM_NUM_CONTRAT  NUMERIC,
    DAT_CONTRAT      DATE NOT NULL,
    DAT_ECH          DATE,
    COD_MOTIF        VARCHAR(4),
    COD_DEPT         VARCHAR(4),
    COD_SERV         VARCHAR(10),
    MNT_MENS         NUMERIC(10,3),
    COD_EVA          VARCHAR(4),
    DATE_EVA         DATE,
    COD_NIVEAU       VARCHAR(5),
    VALID            VARCHAR(1),
    DUREE            BIGINT,
    COD_FILL         VARCHAR(10),
    NUM_ORD          BIGINT,
    ...
    DAT_CREATION     DATE,
    DAT_VALID        DATE,
    NOUV_MOTIF       VARCHAR(4),
    DUREE_ESSAI      INTEGER,
    COD_AFFECT       VARCHAR(4),
    NUM_MVT          INTEGER,
    COD_ALERT        VARCHAR(4),
    SEQ_ALERT        BIGINT,
    NADM_TECH        VARCHAR(10),
    NCOD_AFFECT      VARCHAR(4),
    NCOD_SERV        VARCHAR(10),
    NBR_PTS          NUMERIC,
    COD_CAT          VARCHAR(4),
    COD_GRAD         VARCHAR(4),
    COD_CATEG        VARCHAR(4),
    ...
    NIV_SAL          INTEGER,
    DAT_RUPTURE      DATE,
    TYP_SALAIRE      VARCHAR(1),
    NCOD_CATEG       VARCHAR(4),
    NCOD_CAT         VARCHAR(4),
    NCOD_GRAD        VARCHAR(4),
    NCOD_ECH         SMALLINT,
    NCOD_STAT        VARCHAR(4),
...
CREATE TABLE MOUVEMENT_PERS (
    COD_SOC          VARCHAR(4)   NOT NULL,
    MAT_PERS         VARCHAR(10)  NOT NULL,
    NUM_MVT          BIGINT  NOT NULL,
    DAT_MVT          DATE,
    DAT_EFFET        DATE,
    DAT_DECISION     DATE,
    NUM_DECISION     VARCHAR(20),
    ETAT_MVT         VARCHAR(1),
    COD_MVT          VARCHAR(4),
    COD_DEPT         VARCHAR(4),
    COD_SERV         VARCHAR(10),
    ...
    NCOD_DEPT        VARCHAR(4),
    NCOD_SERV        VARCHAR(10),
    NORG_SERV        VARCHAR(15),
    NCOD_LIEU_GEOG   VARCHAR(4),
    NQUALF           VARCHAR(4),
    NADM_TECH        VARCHAR(10),
    NCOD_CLASS       VARCHAR(4),
    NCOD_FIL         VARCHAR(4),
    NCOD_CATEG       VARCHAR(4),
    NCOD_CAT         VARCHAR(4),
    NCOD_GRAD        VARCHAR(4),
    NCOD_ECH         VARCHAR(4),
    NCOD_FONCT       VARCHAR(4),
    NCOD_AFFECT      VARCHAR(4),
    NCOD_MOTIF       VARCHAR(4),
    NCOD_TYP_DEPART  VARCHAR(4),
    NCOD_METIER      VARCHAR(4),
    NCOD_POST        VARCHAR(15),
    NNIV_SAL         INTEGER,
    REF_MVT          VARCHAR(1),
    ORDONNANCE       VARCHAR(1) DEFAULT 'N',
    ETAT_ACT         VARCHAR(1),
    VAL_PERS         VARCHAR(1),
    REG_POST         VARCHAR(4),
    NREG_POST        VARCHAR(4),
    ID_MOUVEMENT_PERS NUMERIC,
...
CREATE TABLE AFFIL_PERS (
    ...
    TYP_AFFIL        VARCHAR(15),
    DAT_AFFIL        DATE,
    ID_AFFIL_PERS    NUMERIC,
    DAT_FIN          DATE,
    NUM_AFFIL        VARCHAR(20),
    COD_USER         VARCHAR(10),
...
CREATE TABLE ATTEST_TRAV (
    ...
    EDIT             VARCHAR(1) DEFAULT 'N',
    RESPONSABLE      VARCHAR(100),
    MOTIF_A          VARCHAR(100),
    AFFECTATION_A    VARCHAR(100),
    RESPONSABLE_A    VARCHAR(100),
    TYP_ATT          VARCHAR(2)  NOT NULL,
    COD_USER         VARCHAR(10),
    COD_AFF          VARCHAR(4),
    TNET             NUMERIC(15,3),
    ID_ATTEST_TRAV   NUMERIC,
    DAT_DEM          DATE,
    NUM_DEC          VARCHAR(10),
    DAT_DEC          DATE,
    NB_COEF          NUMERIC(5,2),
    NBR_MOIS         SMALLINT,
    DECOUV_DEM       NUMERIC(15,3),
...
CREATE TABLE CONSEIL_ADMINISTRATION (
    COD_SOC          VARCHAR(4)  NOT NULL,
    REF_REG          VARCHAR(40) NOT NULL,
    DAT_CONSEIL      DATE,
    MAT_PERS         VARCHAR(10),
    ID_CONSEIL_ADMINISTRATION NUMERIC,
...
CREATE TABLE TYP_CONGE (
    TYP_CNG          VARCHAR(4)  NOT NULL,
    LIB_CNG          VARCHAR(40),
    MOIS_DEBUT       VARCHAR(5),
    MOIS_FIN         VARCHAR(5),
    RESERVE          VARCHAR(1),
    LIB_CNG_A        VARCHAR(40),
    DROIT_CONGE      VARCHAR(1),
    NAT_TYP_CNG      VARCHAR(1),
    SOLD             VARCHAR(1),
...
CREATE TABLE REGIME_CONGE (
    REG_CNG          VARCHAR(4)  NOT NULL,
    LIB_REG_CNG      VARCHAR(40),
    LIB_REG_CNG_A    VARCHAR(40),
    TAUX_REG_CNG     NUMERIC(6,1),
    TYP_PLAFOND      VARCHAR(4),
...
CREATE TABLE CYCLE_CONGE (
    COD_CYCL         VARCHAR(4)  NOT NULL,
    DAT_DEBUT_CYCL   VARCHAR(5),
    DAT_FIN_CYCL     VARCHAR(5),
...
CREATE TABLE CONDITION_CONGE (
    COD_COND         VARCHAR(4)  NOT NULL,
    LIB_COND         VARCHAR(60),
    ANC_COND         INTEGER,
    REG_CNG          VARCHAR(4)  NOT NULL,
    ID_CONDITION_CONGE NUMERIC,
...
CREATE TABLE CRITERE_CONGE (
    REG_CNG          VARCHAR(4)  NOT NULL,
    NOM_TABLE        VARCHAR(40) NOT NULL,
    NOM_COL          VARCHAR(40) NOT NULL,
    CONDITION_CRITERE VARCHAR(40),
    VALEUR_CRITERE   VARCHAR(200),
    ORDRE            INTEGER  NOT NULL,
    OPERATOR         VARCHAR(5),
    COD_COND         VARCHAR(4)  NOT NULL,
    ID_CRITERE_CONGE NUMERIC,
...
CREATE TABLE CRITERE_CYCLE_CONGE (
    ...
    NOM_TABLE        VARCHAR(40) NOT NULL,
    NOM_COL          VARCHAR(40) NOT NULL,
    CONDITION_CRITERE VARCHAR(40) NOT NULL,
    VALEUR_CRITERE   VARCHAR(200) NOT NULL,
    ORDRE            INTEGER  NOT NULL,
    OPERATOR         VARCHAR(5),
    ID_CRITERE_CYCLE_CONGE NUMERIC,
...
CREATE TABLE JOURS_FERIERS (
    COD_FERIE        SMALLINT  NOT NULL,
    REGIME_TRAV      VARCHAR(1)  NOT NULL,
    LIB_FERIE        VARCHAR(40),
    TYP_FERIE        VARCHAR(1),
    COD_M            VARCHAR(4),
    LIB_FERIE_A      VARCHAR(40),
    RECUP_F          VARCHAR(1),
    ABRV_FIXE        VARCHAR(10),
    ID_JOURS_FERIERS NUMERIC,
...
CREATE TABLE TYP_AUTORISATION (
    COD_TYP_AUT      VARCHAR(4)  NOT NULL,
    LIB_TYP_AUT      VARCHAR(50),
    LIB_TYP_AUT_A    VARCHAR(50),
    NAT_TYP_AUT      VARCHAR(4),
    PLAF_PAY          SMALLINT,
...
CREATE TABLE CYCLE_POINTAGE (
    ...
    LIB_CYCLE        VARCHAR(40),
    ID_CYCLE_POINTAGE NUMERIC,
...
CREATE TABLE CARTE_POINT (
    N_CARTE          BIGINT NOT NULL,
    ETAT_CARTE       VARCHAR(1)  NOT NULL,
    COD_SOC          VARCHAR(4)  NOT NULL,
    BORNE_INF        BIGINT,
    BORNE_SUP        BIGINT,
    ID_CARTE_POINT   NUMERIC,
...
CREATE TABLE REGIME (
    ...
    LIB_REGIME       VARCHAR(40),
    TAUX_SAL         NUMERIC(5,2),
    LIB_REGIME_A     VARCHAR(40),
    TYPE             VARCHAR(1),
    COD_SOC          VARCHAR(4)  NOT NULL,
    NAT_REG          VARCHAR(1),
    TYP_REG          VARCHAR(1),
    TOL_ENTREE       INTEGER,
    TOL_SORTIE       INTEGER,
    TOL_HS_ENTREE    INTEGER,
    TOL_HS_SORTIE    INTEGER,
    ID_REGIME        NUMERIC,
...
CREATE TABLE REGIME_REGIME (
    COD_SOC          VARCHAR(4)  NOT NULL,
    COD_REG          VARCHAR(4)  NOT NULL,
    REG_COD_REG      VARCHAR(4)  NOT NULL,
    DAT_DEBUT        DATE         NOT NULL,
    DAT_FIN          DATE         NOT NULL,
    ID_REGIME_REGIME NUMERIC,
...
-- ---------------------------------------------------------------------------
CREATE TABLE SHIF (
    COD_SHIFT        VARCHAR(4)  NOT NULL,
    MARG_SHIFT       VARCHAR(4),
    LIB_SHIFT        VARCHAR(40) NOT NULL,
    ABR_SHIFT        VARCHAR(10),
    PAUSE_SHIFT      INTEGER,
    H_ENTREE         SMALLINT,
    M_ENTREE         SMALLINT,
    H_SORTIE         SMALLINT,
    ...
    PLAGE_AP_S       NUMERIC,
    PLAGE_AP_E       NUMERIC,
    LIB_SHIFT_A      VARCHAR(40),
    TYP_REG          VARCHAR(1),
    COD_SOC          VARCHAR(4)  NOT NULL,
    ID_SHIF          NUMERIC,
    TYPE_SHIF        VARCHAR(1)  DEFAULT 'S',
    PLAGE_A_E_C      NUMERIC       DEFAULT 0,
    PLAGE_A_S_C      NUMERIC       DEFAULT 0,
    PLAGE_AP_S_C     NUMERIC       DEFAULT 0,
    PLAGE_AP_E_C     NUMERIC       DEFAULT 0,
...
CREATE TABLE SHIFT_REG (
    ...
    BASE_H           SMALLINT,
    BASE_M           SMALLINT,
    JOURS_SEM        SMALLINT  NOT NULL,
    TYPE             VARCHAR(1),
    PAUSE            NUMERIC(4,2),
    MARGE            NUMERIC(4,2),
    COD_SOC          VARCHAR(4)  NOT NULL,
    ID_SHIFT_REG     NUMERIC,
...
CREATE TABLE MOTIF_J (
    ...
    LIM_DEM_CNG      INTEGER,
    UNIT_PLAF        VARCHAR(1),
    PLAFOND_CNG_DEMI INTEGER,
    PLAFOND_CNG_SOLD INTEGER,
    PLAFOND_DECE     VARCHAR(1),
    ORD_REND         SMALLINT,
    RESERVE          VARCHAR(1),
    PLAFOND_ABAT     BIGINT,
    MOIS_DEBUT       VARCHAR(5),
    MOIS_FIN         VARCHAR(5),
    PLAFOND_BONUS    NUMERIC(10,2),
    LIB_MOT_A        VARCHAR(40),
    ABRV_MOT_A       VARCHAR(10),
    DED_CNR          INTEGER,
    NATURE_CNG       VARCHAR(1),
    CORRES_COD_M     VARCHAR(10),
    MOTIF_POINT      VARCHAR(2),
    TYPE_ABS         VARCHAR(4),
    CORRES_PERS      VARCHAR(2),
    NAT_CNG          VARCHAR(1),
    MOTIF_CHEV_POINT VARCHAR(1),
    NBR_JOUR_MIN     NUMERIC(5,2),
    CUMUL_HSUP48     VARCHAR(1),
    DED_ANCIENNETE   VARCHAR(1),
    UNITE_POINT      VARCHAR(2),
    COEF_POINT       SMALLINT,
    COD_SIT          VARCHAR(1),
    SEXE             VARCHAR(1),
    DED_TICKET       VARCHAR(1),
    FRANCH_TICKET    VARCHAR(6),
    COD_ALERT        VARCHAR(4),
    ABAT_PSPES       VARCHAR(1),
    NUM_DEC          NUMERIC,
    ENFANT           VARCHAR(1),
    NUM_DEC_REP      NUMERIC,
...
CREATE TABLE JUSTIF_MOTIF (
    COD_JUSTIF       VARCHAR(4)  NOT NULL,
    COD_M            VARCHAR(4)  NOT NULL,
    OBSERV_JUSTIF    VARCHAR(40),
    TYP_CONTROL      VARCHAR(1),
    LIMIT_CONTROL    INTEGER,
    OBSERV_JUSTIF_A  VARCHAR(40),
    ID_JUSTIF_MOTIF  NUMERIC,
...
CREATE TABLE PLAF_MOTIF_CNG (
    COD_M            VARCHAR(4)  NOT NULL,
    COD_AFFECT       VARCHAR(4)  NOT NULL,
    ANCIEN_MIN       INTEGER  NOT NULL,
    ANCIEN_MAX       INTEGER,
    PLAF_PT          INTEGER,
    PLAF_T50         INTEGER,
    PLAF_T100        INTEGER,
    UNIT_PLAF        VARCHAR(1),
    ID_PLAF_MOTIF_CNG NUMERIC,
...
CREATE TABLE SOLD_CNG (
    COD_SOC          VARCHAR(4)  NOT NULL,
    MAT_PERS         VARCHAR(10) NOT NULL,
    ANNEE_CNG        INTEGER  NOT NULL,
    TYP_CNG          VARCHAR(4)  NOT NULL,
    INIT_CNG         NUMERIC(6,3),
    CUM_CNG          NUMERIC(6,3),
    PRIS_CNG         NUMERIC(7,3),
    FREQUENCE        INTEGER,
    OBS_CNG          VARCHAR(80),
    SEM_CNG          VARCHAR(1),
    SOLD_CNG         NUMERIC(7,3),
    CUM_CNG_ANN_1   NUMERIC(7,3),
    CNG_NJUSTIF      SMALLINT,
    CNG_JUSTIF       SMALLINT,
    NOT_1_PRIS       VARCHAR(1),
    NOT_2_PRIS       VARCHAR(1),
    BONUS_CNG        NUMERIC(6,3),
    OBS_CNG_A        VARCHAR(80),
    SOLD_3112        NUMERIC(7,2),
    BRUT_3112        NUMERIC(10,3),
    SERV_3112        VARCHAR(10),
    RECUP_CNG        NUMERIC(6,3),
    CHARG_3112       NUMERIC(10,3),
    NBR_HEURS_PRIS   NUMERIC,
    NBR_HEURS_INIT   NUMERIC,
    NBR_HEURS_SOLD   NUMERIC,
    VALID_DROIT      VARCHAR(1)  DEFAULT 'N',
    CNG_PAYE         NUMERIC(7,3),
    ID_SOLD_CNG      NUMERIC,
...
CREATE TABLE PLANING_CNG (
    ...
    ANNEE_CNG        INTEGER  NOT NULL,
    TYP_CNG          VARCHAR(4)  NOT NULL,
    AVANCE_CNG       NUMERIC(6,3),
    INIT_CNG         NUMERIC(6,3),
    CUM_CNG          NUMERIC(6,3),
    CNG_PRIS         NUMERIC(6,3),
    CATEG            VARCHAR(4),
    CAT              VARCHAR(4),
    GRAD             VARCHAR(4),
    DEPT             VARCHAR(4),
    SERV             VARCHAR(10),
    CUM_CNG_ANN_1   NUMERIC(6,3),
    BONUS_CNG        NUMERIC(6,3),
    SOLD_CNG         NUMERIC(6,3),
    ID_PLANING_CNG   NUMERIC,
    ETAT_PLANING     VARCHAR(2)  DEFAULT 'I',
    DAT_DEBUT        DATE,
...
CREATE TABLE DETAIL_PLANING_CNG (
    ...
    NBR_JOURS        NUMERIC(6,3),
    NBR_JOURS_CAL    NUMERIC(6,3),
    MAT_PERS_INT     VARCHAR(10),
    OBS              VARCHAR(100),
    OBS_A            VARCHAR(100),
    ID_DETAIL_PLANING_CNG NUMERIC,
    NUM_DCNG         INTEGER,
...
CREATE TABLE VERIF_PLANING_CNG (
    ...
    DATE_FIN         DATE         NOT NULL,
    TYP_ERROR        VARCHAR(2),
    LIB_ERROR        VARCHAR(100),
    LIB_ERROR_A      VARCHAR(100),
    ID_VERIF_PLANING_CNG NUMERIC,
...
CREATE TABLE RECUP_SOLD_CNG (
    ...
    NUM_DEM          NUMERIC,
    DAT_DEM          DATE,
    NBR_JOUR_DEM     NUMERIC(7,3),
    ETAT_DEM         VARCHAR(4),
    DAT_ETAT_DEM     DATE,
    OBS_DEM          VARCHAR(200),
    NBR_JOUR         NUMERIC(7,3),
    ID_RECUP_SOLD_CNG NUMERIC,
...
CREATE TABLE DEM_CNG (
    ...
    DAT_NBO_RET      DATE,
    CNG_PROL         VARCHAR(1),
    NUM_DCNG_PROL    INTEGER,
    NBR_JOUR_PROL    NUMERIC,
    PLANIFIER        VARCHAR(1),
    NAT_CNG          VARCHAR(4),
    CNG_TEMPS        VARCHAR(1),
    CNG_TEMPS_DEBUT  VARCHAR(1),
    CNG_TEMPS_FIN    VARCHAR(1),
    NBR_JOURS_DEMI   NUMERIC(6,3),
    NBR_JOURS_SOLD   NUMERIC(6,3),
    DAT_JOURS_DEMI   DATE,
    DAT_JOURS_SOLD   DATE,
    COD_UR           VARCHAR(4),
    COD_SERV         VARCHAR(10),
    ...
    COD_AFFECT       VARCHAR(4),
    COD_CLASS        VARCHAR(4),
    REGIME_TRAV      VARCHAR(4),
    REG_CNG          VARCHAR(4),
    COD_CYCL         VARCHAR(4),
    RESIDENCE_A      VARCHAR(1000),
    MOTIF_A          VARCHAR(50),
    NUM_SANC         INTEGER,
    NUM_ACC          VARCHAR(10),
    NUM_MAL          VARCHAR(10),
    ...
    COD_ALERT        VARCHAR(4),
    SEQ_ALERT        BIGINT,
    SOLD_CNG         NUMERIC(10,3),
    CNG_VISIT        VARCHAR(1),
    DAT_CONTRE_VISIT DATE,
    CNG_INIT         VARCHAR(1),
    AVANCE_CNG       NUMERIC,
    NBR_HEURE        NUMERIC,
    NOM_MEDCIN       VARCHAR(100),
    CUM_RETENUE      NUMERIC(7,3),
    RETENUE_MOIS     NUMERIC(7,3),
    CUM_RAPPEL       NUMERIC(7,3),
    COD_CATEG        VARCHAR(4),
    COD_CAT          VARCHAR(4),
    COD_GRAD         VARCHAR(4),
    SIGN_CNG         VARCHAR(1),
    ...
    CLOTURE          VARCHAR(1),
    INJECT           VARCHAR(1),
    COD_USER_CONF    VARCHAR(10),
    DAT_USER_CONF    DATE,
    ID_DEM_CNG       NUMERIC,
    NBR_JOURS_OUV    NUMERIC(7,3),
...
CREATE TABLE JUSTIF_DEM_CNG (
    ...
    NUM_DCNG         INTEGER  NOT NULL,
    DAT_JUSTIF       DATE,
    NUM_JUSTIF       VARCHAR(20),
    OBSERV_JUSTIF    VARCHAR(40),
    DAT_PIECE        DATE,
    COD_M            VARCHAR(4),
    ID_JUSTIF_DEM_CNG NUMERIC,
...
CREATE TABLE INTERIM_CNG (
    ...
    MAT_PERS         VARCHAR(10) NOT NULL,
    NUM_DCNG         INTEGER  NOT NULL,
    MAT_PERS_INT     VARCHAR(10) NOT NULL,
    DAT_DEBUT_INT    DATE,
    DAT_FIN_INT      DATE,
    DAT_DECISION     DATE,
    NUM_DECISION     VARCHAR(10),
    NATURE_INT       VARCHAR(1),
    ORDONANCE        VARCHAR(1),
    MNT_INT          NUMERIC(10,3),
    COD_UR           VARCHAR(4),
    COD_SERV         VARCHAR(10),
    COD_MOTIF        VARCHAR(4),
    COD_AFFECT       VARCHAR(4),
...
CREATE TABLE PERS_CARTE (
    ...
    DATE_OPT         DATE,
    CART_COUR        VARCHAR(1),
    ETAT_CARTE       VARCHAR(1),
    ID_PERS_CARTE    NUMERIC,
...
CREATE TABLE AUTORISATION_SORT (
    COD_SOC          VARCHAR(4)  NOT NULL,
    MAT_PERS         VARCHAR(10) NOT NULL,
    NUM_AUT          NUMERIC       NOT NULL,
    DAT_DEBUT_AUT    DATE         NOT NULL,
    DAT_FIN_AUT      DATE,
    COD_AUT          VARCHAR(4)  NOT NULL,
    ETAT_AUT         VARCHAR(1),
    HEUR_S           NUMERIC,
    MIN_S            NUMERIC,
    HEUR_R           NUMERIC,
...
CREATE TABLE POINTER_IND (
    MAT_PERS         VARCHAR(10),
    H_POINT          VARCHAR(2)  DEFAULT '0' NOT NULL,
    MIN_POINT        VARCHAR(2)  DEFAULT '0' NOT NULL,
    MONTHS           VARCHAR(2)  NOT NULL,
    YEARS            VARCHAR(2)  NOT NULL,
    DAYS             VARCHAR(2)  NOT NULL,
    N_CARTE          VARCHAR(10) NOT NULL,
    COD_SOC          VARCHAR(4)  NOT NULL,
    NUM_POINTEUSE    VARCHAR(4),
    NAT_POINT        VARCHAR(1),
    COD_REG          VARCHAR(4),
    ETAT_IND         VARCHAR(1)  DEFAULT 'I',
    NUM_POINT        NUMERIC,
    TYP_VALID        VARCHAR(1),
    SS_POINT         VARCHAR(2),
    COD_M            VARCHAR(8),
    ID_POINTER_IND   NUMERIC,
...
CREATE TABLE POINTER (
    ...
    DAT_SAIS_SYS     DATE,
    TOT_RET          NUMERIC(5,2),
    RET_HEUR         NUMERIC(5,2),
    RET_MIN          NUMERIC(5,2),
    ID_POINTER       NUMERIC,
    INTERMED         VARCHAR(4),
    S_POINT          INTEGER,
    OLD_TYP_POINT    VARCHAR(2),
    DAT_JOUR         DATE,
    YEARS_           INTEGER,
    MONTHS_          SMALLINT,
    DAYS_            SMALLINT,
    SERV_POINT       VARCHAR(10),
    DUREE_H          SMALLINT,
    DUREE_M          SMALLINT,
    DUREE_TOT        VARCHAR(20),
    TEMP_COMP        NUMERIC,
...
CREATE TABLE RETARD_JOURNEE (
    ...
    COD_REG          VARCHAR(4),
    DATE_SHIFT       DATE,
    ETAT_RETARD      VARCHAR(1),
    DAT_SAIS         DATE,
    ID_RETARD_JOURNEE NUMERIC,
    H_ENTRE          INTEGER,
    M_ENTRE          INTEGER,
    TYP_SAIS         VARCHAR(1)  DEFAULT 'A',
    TYP_POINT        VARCHAR(1),
    TRANSF           VARCHAR(1),
    COD_AUT          VARCHAR(4),
    DURE_AUT         NUMERIC,
...
CREATE TABLE CAL_PERS (
    ...
    COD_M            VARCHAR(4),
    SEMAINE          SMALLINT,
    PREC_COD_M       VARCHAR(4),
    BASE_H           NUMERIC,
    BASE_M           NUMERIC,
    PAUSE_SHIFT      NUMERIC(4,2),
    MARG_SHIFT       NUMERIC(4,2),
    COD_REG          VARCHAR(4),
    COD_SERV         VARCHAR(10),
    PRES_H           NUMERIC(5,2),
    PRES_M           NUMERIC(5,2),
    COD_FIL          VARCHAR(10),
    REGIME_TRAV      VARCHAR(1),
    REGIME_HEUR      SMALLINT,
    DROIT_ASTR       VARCHAR(1)  DEFAULT 'N',
    DT_BUL           DATE,
    COD_TYP_BUL      VARCHAR(4),
    ID_CAL_PERS      NUMERIC,
    UNITE            VARCHAR(1),
    SEANCE           VARCHAR(1)  DEFAULT 'J',
...
CREATE TABLE TYPE_BONIFICATION (
    COD_MOTIF_BONIF      VARCHAR(4)     NOT NULL,
    LIB_MOTIF_BONIF      VARCHAR(120),
    LIB_MOTIF_BONIF_A    VARCHAR(120),
    TAUX                 NUMERIC,
...
CREATE TABLE MODELE_EVALUATION (
    NUM_MODELE       VARCHAR(10)    NOT NULL,
    LIB_MODELE       VARCHAR(40),
    LIB_MODELE_A     VARCHAR(40),
    TYP_MODELE       VARCHAR(1),
...
CREATE TABLE EPREUVE (
    CODE_EPREUVE     VARCHAR(10)    NOT NULL,
    LIB_EPREUVE      VARCHAR(40),
    TYP_EPREUVE      VARCHAR(1),
    ABRV_EPREUVE     VARCHAR(10),
    LIB_EPREUVE_A    VARCHAR(40),
...
-- ----------------------------------------------------------------------------
CREATE TABLE SALLE (
    COD_SALLE        VARCHAR(2)     NOT NULL,
    LIB_SALLE        VARCHAR(60)    NOT NULL,
    CAPACITE         NUMERIC,
    LIB_SALLE_A      VARCHAR(60),
    COD_ETAB_CONC    VARCHAR(4)     NOT NULL,
    ID_SALLE         NUMERIC,
...
CREATE TABLE LISTE_DIVERS (
    CODE_DIVERS      VARCHAR(10)    NOT NULL,
    LIB_DIVERS       VARCHAR(50),
    LIB_DIVERS_A     VARCHAR(50),
...
CREATE TABLE VALEUR_CRITERE (
    COD_VAL          VARCHAR(4)     NOT NULL,
    LIB_VAL          VARCHAR(80),
    LIB_VAL_A        VARCHAR(80),
...
-- ----------------------------------------------------------------------------
CREATE TABLE AVIS (
    NUM_AVIS         VARCHAR(6)     NOT NULL,
    LIBELLE_AVIS     VARCHAR(40),
    DAT_AVIS         DATE,
    DAT_LIMIT        DATE,
    TYPE_AVIS        VARCHAR(1),
    SITE_INSCRIP     VARCHAR(100),
    ADR_CORRESP      VARCHAR(300),
    BUR_REGIONAL     VARCHAR(100),
    ETAT_AVIS        VARCHAR(1),
    SUJET_AVIS       VARCHAR(300),
    COD_SOC          VARCHAR(4),
...
CREATE TABLE CONCOURS (
    CODE_CONCOURS    VARCHAR(10)    NOT NULL,
    LIB_CONCOURS     VARCHAR(40),
    COD_ETAB_CONC    VARCHAR(10),
    DATE_CONCOURS    DATE,
    TYPE_CONCOURS    VARCHAR(1)     NOT NULL,
    HEUR_C           DATE,
    LIEU_C           VARCHAR(60),
    ETAT1            VARCHAR(2),
    MOY_CONCOURS     NUMERIC(5,2),
    NUM_AVIS         VARCHAR(6)     NOT NULL,
    ORDR_AVIS        NUMERIC          NOT NULL,
    CODE_DOMAINE     VARCHAR(10)    NOT NULL,
    TYPE_REUSSITE    VARCHAR(1),
    NAT_SELECT       VARCHAR(1),
    ETAT_CONC        VARCHAR(1),
    BONIFICATION     VARCHAR(1),
    CONC_COD_CONC    VARCHAR(10),
    COEF             SMALLINT,
    COD_CAT_CLASS    VARCHAR(4),
    COD_SOC          VARCHAR(4),
...
CREATE TABLE FICHE_CANDIDAT (
    NUM_FICHE            VARCHAR(15)    NOT NULL,
    COD_SOC              VARCHAR(4),
    MAT_PERS             VARCHAR(10),
    NOM_CAND             VARCHAR(40),
    PREN_CAND            VARCHAR(40),
    TYP_CAND             VARCHAR(1),
    NOM_JF               VARCHAR(80),
    DAT_NAIS             DATE,
    ...
    TYP_IDENTIFIANT      VARCHAR(1),
    DAT_EMI_IDENT        DATE,
    LIEU_EMI_IDENT       VARCHAR(100),
    NUM_PERMIS           VARCHAR(10),
    DAT_OBT_PERMIS       DATE,
    SIT_FAM              VARCHAR(1),
    NBR_ENF              NUMERIC,
    PROF_CONJ            VARCHAR(30),
    ORG_PROF_CONJ        VARCHAR(50),
    TAILLE               NUMERIC,
    POIDS                NUMERIC,
    GRP_SANGUIN          VARCHAR(3),
    ETAT_SANTE           VARCHAR(1),
    HANDICAP             VARCHAR(1)     DEFAULT NULL,
    NOM_HANDICAP         VARCHAR(100),
    ANTECED_JUDICIAIRE   VARCHAR(1),
    DAT_ANTECED_JUD      DATE,
    SERV_MILITAIRE       VARCHAR(20),
    ACTION_PREVUE        VARCHAR(100),
    DAT_CREAT_FICHE      DATE,
    DAT_MAJ_FICHE        DATE,
    ETAT_FICHE           VARCHAR(1),
    COD_SOURCE           VARCHAR(6),
    OBSERV               VARCHAR(100),
    COD_SALLE            VARCHAR(2),
    CODE_CONCOURS        VARCHAR(10),
    MOYEN_FINAL          NUMERIC          DEFAULT NULL,
    RESULT_FINAL         VARCHAR(1),
    CLASSEMENT           INTEGER       DEFAULT NULL,
    CODE_MOYEN           VARCHAR(2),
    ...
    COD_SERV             VARCHAR(10),
    VM                   VARCHAR(1)     DEFAULT NULL,
    DF                   VARCHAR(1)     DEFAULT NULL,
    PERE                 VARCHAR(10),
    ANC_MAT              VARCHAR(10),
    COD_MOTIF_BONIF      VARCHAR(4),
    COD_REFUS            VARCHAR(10),
    DAT_DEPOS_DEM        DATE,
    NUM_CACHET           NUMERIC,
    ORG_SERV             VARCHAR(15),
    COD_METIER           VARCHAR(4),
    COD_POST             VARCHAR(15),
    COD_GOUV             VARCHAR(4),
    POURCENT_HAND        NUMERIC(5,2),
    TYP_HANDICAP         VARCHAR(1),
    NUM_FICH_HAND        VARCHAR(10),
    COEF_COMP            INTEGER,
    COD_NAT_RECR         VARCHAR(4),
    COD_TYP_CONT         VARCHAR(4),
    REQUETE_ADM          VARCHAR(1),
    CASIER_JUD           VARCHAR(1),
    OBS_ENQUETE          VARCHAR(500),
    DAT_REQUETE          DATE,
    COD_PROMO            VARCHAR(4),
    F_MOJHD_CHEHID       VARCHAR(1),
    ADM_TECH             VARCHAR(10),
    ADRESSE_A            VARCHAR(100),
    ID_FICHE_CANDIDAT    NUMERIC,
    NOM_CAND_A           VARCHAR(40),
    PREN_CAND_A          VARCHAR(40),
...
CREATE TABLE STAGE (
    ...
    DAT_FIN_PROLG    DATE,
    TYP_STAGE        VARCHAR(1),
    PERIODE_PROLG    NUMERIC,
    DAT_QUIT         DATE,
    DAT_TIT          DATE,
    TYP_DEPART       VARCHAR(4),
    NUM_RETR         VARCHAR(10),
    DAT_MEMOIR       DATE,
    DAT_REM_MEM      DATE,
    SUJET_MEM        VARCHAR(300),
    ENCADREUR        VARCHAR(100),
    OBJET_STAGE      VARCHAR(300),
    RESULT_STAGE     VARCHAR(1),
    COD_ALERT        VARCHAR(4),
    SEQ_ALERT        BIGINT,
    ID_STAGE         NUMERIC,
    DAT_DECISION     DATE,
...
CREATE TABLE DETAIL_CONTRAT (
    ...
    DAT_DEBUT        DATE            NOT NULL,
    DAT_FIN          DATE,
    SALAIRE          NUMERIC(12,3),
    COD_STAT         VARCHAR(4),
    ID_DETAIL_CONTRAT NUMERIC,
...
CREATE TABLE PERSONNEL_RECRUT (
    ...
    COD_NIVEAU           VARCHAR(5),
    NOM_JF               VARCHAR(80),
    NOM_PERS_A           VARCHAR(60),
    PREN_PERS_A          VARCHAR(60),
    POSTE_TRAV           VARCHAR(15),
    COD_METIER           VARCHAR(4),
    COD_USER             VARCHAR(10),
    DAT_MAJ              DATE,
    ...
    DAT_STAT             DATE,
    DAT_CIN              DATE,
    LIE_EMI_CIN          VARCHAR(100),
    DAT_AFFECT           DATE,
    LIEU_NAIS            VARCHAR(200),
    COD_LIEU_GEOG        VARCHAR(10),
    DAT_LIEU_GEOG        DATE,
    GRP_SANG             VARCHAR(3),
    NUM_RETR             VARCHAR(12),
    COD_FIL              VARCHAR(4),
    ...
    NIV_SAL              INTEGER,
    COD_ASSUR            VARCHAR(4),
    NUM_ASSUR            VARCHAR(10),
    COD_RETR             VARCHAR(5),
    TYP_ID               VARCHAR(1),
    ETAT_SANTE           VARCHAR(1),
    DAT_EFF_FICH_HAND    DATE,
    DAT_FIN_FICH_HAND    DATE,
    REF_FONCT            VARCHAR(20),
    NAT_TEXTE_FONCT      VARCHAR(2),
    NOUV_DAT_ECH         DATE,
    DAT_NIV_SAL          DATE,
    DAT_ASS              DATE,
    DAT_AFF_CNAM         DATE,
    ETAT_POSTE_TRAV      VARCHAR(1),
    ETAT                 VARCHAR(1),
    COD_TYP_CONT         VARCHAR(4),
    COD_BANQ             VARCHAR(4),
    COD_AGC              VARCHAR(5),
    CPT_BANQ_PERS        VARCHAR(25),
    ID_PERSONNEL_RECRUT  NUMERIC,
    NUM_DECIS            VARCHAR(20),
    DAT_DECIS            DATE,
...
CREATE TABLE DIVERS_AVIS (
    ...
    ETAT             VARCHAR(1),
    NBR_DIVERS_AVIS  NUMERIC,
    OBLIG            VARCHAR(1),
    COD_SOC          VARCHAR(4),
    ID_DIVERS_AVIS   NUMERIC,
...
CREATE TABLE LINE_CONCOURS (
    ...
    CODE_CONCOURS    VARCHAR(10)    NOT NULL,
    COEF             NUMERIC(4,2),
    NOT_ELIM         NUMERIC(4,2),
    CODE_LANGUE      VARCHAR(10),
    DUREE            NUMERIC,
    SPECIAL          VARCHAR(1),
    DAT_EPREUVE      DATE,
    HEUR_DEB_EPREUVE SMALLINT,
    MIN_DEB_EPREUVE  SMALLINT,
    HEUR_FIN_EPREUVE SMALLINT,
    MIN_FIN_EPREUVE  SMALLINT,
    NBR_NOTE         NUMERIC,
    COD_SOC          VARCHAR(4),
    ID_LINE_CONCOURS NUMERIC,
...
CREATE TABLE SALLE_CONCOURS (
    CODE_CONCOURS    VARCHAR(10)    NOT NULL,
    COD_SALLE        VARCHAR(2)     NOT NULL,
    CAPACITE_REEL    NUMERIC,
    CAPACITE_CONC    NUMERIC,
    CODE_EPREUVE     VARCHAR(10)    NOT NULL,
    COD_ETAB_CONC    VARCHAR(4)     NOT NULL,
    VALIDE           VARCHAR(1),
    COD_SOC          VARCHAR(4),
    ID_SALLE_CONCOURS NUMERIC,
...
CREATE TABLE COMPETENCE_CANDIDAT (
    NUM_FICHE        VARCHAR(15)    NOT NULL,
    COD_NAT_COMP     VARCHAR(4)     NOT NULL,
    COD_COMP         VARCHAR(15)    NOT NULL,
    ID_COMPETENCE_CANDIDAT NUMERIC,
...
CREATE TABLE FORM_CAND (
    ...
    COD_FORM         VARCHAR(10)    NOT NULL,
    DUREE            NUMERIC,
    TYP_DUREE_FC     VARCHAR(1),
    COD_ETAB         VARCHAR(15),
    ID_FORM_CAND     NUMERIC,
...
CREATE TABLE CAND_DIPLOM (
    ...
    CODE_OPTION      VARCHAR(10),
    COD_ETAB         VARCHAR(15),
    TYP_NIVEAU       VARCHAR(1),
    ID_CAND_DIPLOM   NUMERIC,
...
CREATE TABLE DIVERS_CAND (
    ...
    NUM_FICHE        VARCHAR(15)    NOT NULL,
    OBSERVATION      VARCHAR(30),
    OBSERVATION_A    VARCHAR(30),
    COD_SOC          VARCHAR(4),
    ID_DIVERS_CAND   NUMERIC,
...
CREATE TABLE LIGNE_DIR (
    ...
    COD_AFF          VARCHAR(10)    NOT NULL,
    COD_METIER       VARCHAR(4)     NOT NULL,
    COD_POST         VARCHAR(15)    NOT NULL,
    COD_SOURCE       VARCHAR(6),
    NAT_CAND         VARCHAR(1),
    NIV_URGENCE      VARCHAR(1),
    AGE_MIN          SMALLINT,
    AGE_MAX          SMALLINT,
    NBRE_FEM         NUMERIC,
    NBRE_MASC        NUMERIC,
    ...
    VALID            VARCHAR(1),
    OBSERV           VARCHAR(1000),
    SERV_CIBLE       VARCHAR(10),
    ID_LIGNE_DIR     NUMERIC,
...
CREATE TABLE FICHE_EVAL_RECRU (
    ...
    RECOMMAND        VARCHAR(100),
    COD_AFFECT       VARCHAR(4),
    MAT_MEMBRE       VARCHAR(10)    NOT NULL,
    ID_FICHE_EVAL_RECRU NUMERIC,
...
CREATE TABLE DET_FICHE_EVAL_RECRU (
    ...
    MAT_MEMBRE           VARCHAR(10)    NOT NULL,
    ID_DET_FICHE_EVAL_RECRU NUMERIC,
...
CREATE TABLE FICHE_EVAL_STAGE (
    ...
    DAT_DEBUT        DATE            NOT NULL,
    DAT_FICHE        DATE,
    NUM_MODELE       VARCHAR(10),
    OBS              VARCHAR(200),
    TOT              NUMERIC,
    MOYENNE          NUMERIC(5,2),
    ETAT             VARCHAR(1),
    ID_FICHE_EVAL_STAGE NUMERIC,
...
CREATE TABLE DET_EVAL_STAGE (
    ...
    NOTE1            NUMERIC(8,2),
    NOTE2            NUMERIC(8,2),
    APPRECIATION1    VARCHAR(100),
    APPRECIATION2    VARCHAR(100),
    COMMENTAIRE      VARCHAR(100),
    ID_DET_EVAL_STAGE NUMERIC,
...
CREATE TABLE UTILISATEUR (
    ...
    COD_SOC       VARCHAR(4),
    MAT_PERS      VARCHAR(10),                 -- Lien vers PERSONNEL si agent/directeur/RH
    NUM_FICHE     VARCHAR(15),                 -- Lien vers FICHE_CANDIDAT si candidat
    LOGIN         VARCHAR(50)  NOT NULL,
    MOT_PASSE     VARCHAR(255),                -- Hash du mot de passe
    ROLE_USER     VARCHAR(20)  NOT NULL,       -- ADMIN, DIRECTEUR, RH, AGENT, CANDIDAT
    ETAT_USER     VARCHAR(1)   DEFAULT 'A',    -- A=actif, I=inactif, B=bloqué
    DAT_CREATION  DATE          DEFAULT CURRENT_TIMESTAMP,
